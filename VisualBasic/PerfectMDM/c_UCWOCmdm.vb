Public Class c_UCWOCmdm
    ...
    Private Sub c_UCWOCmdm_Load(sender As Object, e As EventArgs) Handles MyBase.Load
...
#Region " Расчет цены "
    ...
    Private Sub CostCalc(quantity As Integer)
        ...
        CostFromPrice = Me.nudY.Value * NonQuery1Scalar("Select dbo.[costUniCalcWOMat](" & Me.cmbPriceName.SelectedItem(0) & "," & Me.cmbParam2.SelectedItem(0) & "," & quantity & ")")
        ...
        ' matCount = RoundUp(quantity * Module1.NonQuery1Scalar("SELECT kolvoSlista FROM PriceParam2 WHERE ID=" & Me.cmbParam2.SelectedItem(0) & ""))
        ...
        matCount = Me.nudY.Value * RoundUp(matCount)
        ...
        If signFerst = False Then ' хотя бы один отмечен
            ...
            costPPO = Me.nudY.Value * Module1.NonQuery1Scalar("SELECT[dbo].[costTechOperCLB] (" & Replace(matCount, ",", ".") & ", '" & strOper & "', 1, " & quantity & ", 0)")
        End If
        ...
        Me.tstxtCostUnit.Text = Format(Math.Round(Itogo / (Me.nudY.Value * quantity), 2), "C")
...
#Region " Добавить расчет к заказу "
    ...
    Function InsertToOrders(ByVal strPrim As String) As Integer
        ...
        Try
            ...
            cmd.Parameters("@prim").Value = "Макетов: " + Me.nudY.Value.ToString + " по " + Me.cmbTirag.Text + " шт. * " + dopOperStr
            ...
            cmd.Parameters("@costUnit").Value = Itogo / (Me.nudY.Value * Val(Me.cmbTirag.Text))  'Replace(Me.tstxtItog.Text / Me.nudQuantity.Value, ",", ".")
            ...
            cmd.Parameters("@kolvo").Value = Val(Me.cmbTirag.Text) * Me.nudY.Value
...
#End Region
    ...
    Private Sub SkidkaLoad()
        If Me.cmbTirag.Text <> Nothing Then
            Try
                Dim odaDiscont As New SqlClient.SqlDataAdapter("SELECT awc_Discont.ID, awc_Discont.Name FROM awc_Discont INNER JOIN awc_DiscontServices ON awc_Discont.ID=awc_DiscontServices.discontID WHERE awc_DiscontServices.serviceID=" & serviceID & " AND quantity<=" & Me.nudY.Value & " AND " & Me.nudY.Value & "<=quantity2 order by awc_Discont.description", mcnnInt)
